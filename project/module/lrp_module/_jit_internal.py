...
def weak_module(cls):
    weak_types[cls] = {"method_stubs": None}
    return cls


def weak_script_method(fn):
    weak_script_methods[fn] = {"rcb": createResolutionCallback(frames_up=2), "original_method": fn}
    return fn
...
try:
    ...
    def is_tuple(ann):
        ...
        return ann.__module__ == "typing" and (
            getattr(ann, "__origin__", None) is typing.Tuple or getattr(ann, "__origin__", None) is tuple
        )

    def is_list(ann):
        return ann.__module__ == "typing" and (
            getattr(ann, "__origin__", None) is typing.List or getattr(ann, "__origin__", None) is list
        )

    def is_dict(ann):
        return ann.__module__ == "typing" and (
            getattr(ann, "__origin__", None) is typing.Dict or getattr(ann, "__origin__", None) is dict
        )
